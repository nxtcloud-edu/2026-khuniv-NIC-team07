...
export default function ExamRegister() {
  ...
  const [form, setForm] = useState({ subject: '', exam_date: futureDate(), exam_time: '09:00', scope_start: 1, scope_end: 200, scope_unit: '페이지', target_passes: 2, priority_chapters: '', planning_preferences: '' })
  ...
  return <Layout>
    ...
    <form className="form-card" onSubmit={submit}>
      <div className="form-grid">
        ...
        <label className="wide">과목 특성 및 계획 요구사항<textarea value={form.planning_preferences} maxLength={4000} onChange={event => setForm({ ...form, planning_preferences: event.target.value })} placeholder={'예: 이 과목은 암기라서 짧게 여러 번 보고 싶어요.\n8월 29일에는 공부하지 않고, 시험 전 이틀은 복습만 배치해 주세요.'} rows={6}/><small>자연어로 자유롭게 적어 주세요. 입력한 내용은 저장되어 최초 계획과 이후 재계획에 계속 반영됩니다.</small></label>
      </div>
