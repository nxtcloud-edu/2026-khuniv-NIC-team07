...
export default function Dashboard() {
  ...
  return <Layout>
    ...
    {detailExam && <div className="modal-backdrop" onMouseDown={()=>setDetailExam(null)}><div className="modal exam-detail-modal" onMouseDown={e=>e.stopPropagation()}><div className="detail-title"><div><span className="kicker">STUDY HISTORY</span><h2>{detailExam.subject} 진행 기록</h2></div><button aria-label="닫기" onClick={()=>setDetailExam(null)}>×</button></div><p className="detail-summary">{detailExam.ai_summary || '계획을 수행하면 변화 기록이 여기에 쌓입니다.'}</p><h3>계획 대비 완수량</h3><div className="completion-chart" aria-label="계획 대비 완수량 그래프">{(detailExam.completion_logs || []).map(log=>{const max=Math.max(log.planned_units,log.completed_units,1);return <div className="chart-row" key={log.id}><span>{new Date(`${log.study_date}T00:00:00`).toLocaleDateString('ko-KR',{month:'numeric',day:'numeric'})}</span><div><i className="planned-bar" style={{width:`${log.planned_units/max*100}%`}}/><i className="done-bar" style={{width:`${log.completed_units/max*100}%`}}/></div><b>{log.completed_units}/{log.planned_units}</b></div>})}{!(detailExam.completion_logs || []).length&&<div className="detail-empty">아직 완수 기록이 없습니다.</div>}</div><div className="chart-legend"><span><i className="planned-dot"/>계획</span><span><i className="done-dot"/>완수</span></div><h3>계획 수정 로그</h3><div className="history-list">{(detailExam.plan_logs || []).map(log=><article key={log.id}><b>계획 v{log.previous_version} → v{log.new_version}</b><time>{new Date(log.created_at).toLocaleString('ko-KR')}</time><p>{log.explanation}</p><small>{log.recommendation}</small></article>)}{!(detailExam.plan_logs || []).length&&<div className="detail-empty">아직 계획 수정 기록이 없습니다.</div>}</div></div></div>}
