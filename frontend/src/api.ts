...
export const api = {
  ...
  updateEvent: (eventId: number, payload: object) => request(`/api/events/${eventId}`, { method: 'PUT', body: JSON.stringify(payload) }),
  deleteEvent: (eventId: number, applyTo: 'THIS'|'SERIES') => request<void>(`/api/events/${eventId}`, { method: 'DELETE', body: JSON.stringify({ apply_to: applyTo }) }),
